...
def knap_solve(w,v,W):
    ...
    for i in range(n):
        ...
        for cap in range(W+1):
            ...
            if wi<=cap:
                cand=dp[i][cap-wi]+vi
                if cand>dp[i+1][cap]: dp[i+1][cap]=cand
    cap=W; chosen=[]
...
def gen_one(r,i):
    ...
    heavy_v=max(120, sum(v)//n)+r.randint(20,80)
    ...
    return {
      ...
      "task_family":"knapsack_01","answer_type":"int","task":"knapsack_01",
      ...
      "hints":[
        ...
        {"level":2,"text":"dp[cap] を容量 cap の最大価値として、各品目を後ろ向きに更新。"},
        {"level":3,"text":"復元は dp の差分から選択アイテムを逆順に辿る。"}
      ],
      "distractors":[
        {"code":"TAKE_MAX_VALUE_ONLY","desc":"価値の大きい順に貪欲に取る誤り"},
        {"code":"WRONG_GREEDY_ORDER","desc":"価値/重さ比のみの貪欲（0/1では最適保証なし）"},
        {"code":"OVERFLOW_EDGE_IGNORED","desc":"容量境界の等号扱いを誤る"}
      ],
      "witness":{"opt_value_t1":opt1,"chosen_items_t1":ch1,"opt_value_t2":opt2,"chosen_items_t2":ch2},
